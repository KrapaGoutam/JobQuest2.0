...
1. **Target Schema Catalog (29 Permanent Tables + 2 Migration Tables = 31 Total Tables):** Reconciled following the adoption of Auth Option B (adding `user_credentials`, `auth_sessions`, `auth_refresh_tokens`, `auth_rate_limits` to the original 25 Gate 03 permanent tables). Complete table definitions, column types, canonical UUIDv4 (`gen_random_uuid()`) primary keys, composite foreign keys, and indexes are detailed in [`../gate-03/TARGET_SCHEMA.md`](../gate-03/TARGET_SCHEMA.md) and [`../gate-03/GATE_03_AUTH_OPTION_B_AMENDMENT.md`](../gate-03/GATE_03_AUTH_OPTION_B_AMENDMENT.md).
...
5. **M1/M1B Foundational Baseline (11 Implemented Tables):** The 7 M1 foundational baseline tables (`user_accounts`, `profiles`, `auth_recovery_codes`, `workspaces`, `workspace_members`, `applications`, `workflow_definitions`) plus the 4 M1B Option B authentication tables (`user_credentials`, `auth_sessions`, `auth_refresh_tokens`, `auth_rate_limits`) are fully applied and verified in `supabase/migrations/`.
