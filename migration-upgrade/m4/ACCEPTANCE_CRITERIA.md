...
- [x] **AC-DB-07 (Closeout integrity, 20260925100000):** direct hard DELETE of contacts/companies denied (42501); interactions append-only; composite company FKs; owner/workspace immutable; peer unlink denied; manager cross-user mutations audited (M4C-01..09).
...
- [x] **AC-DB-07 (Domain RPCs):**
  ...
  - `rpc_log_contact_interaction`: Appends an interaction (append-only table) and touches `contacts.updated_at` and, if given, `next_follow_up_date`. There is no `last_contact_date` column; the UI derives last contact from the newest interaction.
...
- [x] **AC-UI-09 (Accessibility):** Zero critical or serious accessibility violations via axe-core WCAG 2.0/2.1/2.2 A+AA audit (colour contrast included).
